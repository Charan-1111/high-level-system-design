# Long Polling
- Long Polling is a technique that mimics the real-time behaviour by keeping HTTP requests open until the server has data.
- Long Polling is an enhancement over traditional polling. In regular polling, the client repeatedly sends requests at fixed intervals to check for updates. This can be wasteful if no new data exists.
- In long polling , client asks the server for data and then waits until the server has some thing new to return or until a timeout occurs.
...
- ## Working
	...
	- Server holds the request open until it has an update or a timeout is reached.
		...
		- If there's no new data and timeout is reached, the server responds with an empty or minimal message.
